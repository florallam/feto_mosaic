...
if __name__ == "__main__":
    ...
    # load model
    chkpt = "/home/flora/data/DSM_weights_19sept_256both.hdf5"   
    ...
    data_path= "Fetoscopy Placenta Dataset/Vessel_registration_unannotated_clips/video03/cropped_images/"
    save_path= "/home/flora/data/Fetoscopy Placenta Dataset/Vessel_registration_unannotated_clips/video03/results_mosaic/"
    ...
    np.savetxt(save_path+'txt/'+fname, H[0]) 
    ...
    for j in range(no_samples-1):   
        ...
        # print(H_prune.shape)
        # print(len(H_prune))
        # print("\n THIS is gonna be the homography \n")
        Hret = SVD(H_prune)
        ...
        # for saving the relative homography as txt
        np.savetxt(save_path+'/txt/'+fname, Hret) 
        H.append(Hret)
    ...
    for j in range(no_samples):    
        ...
        warped_image = cv2.warpPerspective(color_image,Hrel[j],(int(round(maxyy)),int(round(maxxx))), flags=cv2.INTER_CUBIC,    
                                            borderMode=cv2.BORDER_CONSTANT, borderValue=(0, 0, 0)) #changed border mode from constant to cv2.BORDER_TRANSPARENT
        ...
        cv2.imwrite('Fetoscopy Placenta Dataset/Vessel_registration_unannotated_clips/video03/results_mosaic/images/anon003_'+str("{:05d}".format(j+12661))+'.jpg',mosaic)
        plt.imshow(mosaic)
        plt.show()
        dst = np.copy(mosaic)
