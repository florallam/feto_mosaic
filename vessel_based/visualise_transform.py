'''
    Vessel based method for mosaicking
    After obtaining the affine transformation for image frame to frame, 
    it enables the mosaic to be visualised in a video format
'''
...
def get_mask_im(fullImgPaths, mask_path, crop_top, crop_bottom):
    ...
    # mask_im = mask_im.filter(ImageFilter.MaxFilter(size=13))
...
def generate_video(image_folder, video_name, video_frames_path):
    ...
    frame= frame[1508:2940, 1508:2940] #added this to crop the video
    # frame= frame[1708:3140, 1508:2940] 
    height, width, layers = frame.shape

    fourcc= cv2.VideoWriter_fourcc(*'mp4v') #changed from XVID
    ...
    for i in tqdm(range(len(images))):
        ...
        img= img[1508:2940, 1508:2940] #added this to crop the video
        # img= img[1708:3140, 1508:2940] 
...
def overlay_transparent(bg_img, img_to_overlay_t):
    ...
    #blur edges of mask
    maskk = cv2.GaussianBlur(a,(5,5),0) #added this to blur the edges (attempt to remove black circle around the mask crop)

    # Black-out the area behind the logo in our original ROI
    img1_bg = cv2.bitwise_and(bg_img, bg_img, mask=cv2.bitwise_not(maskk))

    # Mask out the logo from the logo image.
    img2_fg = cv2.bitwise_and(overlay_color, overlay_color, mask = maskk)
...
writepath= "Fetoscopy Placenta Dataset/Vessel_registration_unannotated_clips/video06/masks_vis_results"

seq_name = "anon012" #args.video_seq_name
frames_path = "/home/flora/data/Fetoscopy Placenta Dataset/Vessel_registration_unannotated_clips/video06/masks_transform"
fullImgDirPath = frames_path 
mask_path= "Fetoscopy Placenta Dataset/Vessel_registration_unannotated_clips/video06/mask.png"
miccai_Hpath= "/home/flora/data/Fetoscopy Placenta Dataset/Vessel_registration_unannotated_clips/video06/masks_transform_txt"
...
#added this to rename the output file 
now= datetime.today()
ddmmyy= now.strftime('%d%m%y')
time= now.strftime('%H%M')
# Calling the generate_video function 
generate_video(writepath + '/' + seq_name, seq_name +"_"+ddmmyy+"_"+time+ ".mp4", writepath + '/' + seq_name + "/mosaic") #changed from .avi to .mp4
